...
import { formatCalories, formatWeight, getGoalText, getActivityText, calculateTimeToGoal, calculateWeeklyWeightChange } from '../utils/calculations';
import { Flame, Target, TrendingDown, TrendingUp, Minus, Calendar, Scale, Zap, Activity } from 'lucide-react';
...
export function CalorieResult({ profile, calculation }: CalorieResultProps) {
  ...
  const getGoalColor = () => {
    switch (profile.goal) {
      case 'lose': return 'text-rose border-rose bg-rose/10';
      case 'gain': return 'text-ocean border-ocean bg-ocean/10';
      case 'maintain': return 'text-deep-blue border-deep-blue bg-deep-blue/10';
    }
  };

  const weeklyWeightChange = calculateWeeklyWeightChange(calculation.deficitOrSurplus);
  const timeToGoal = profile.targetWeight ? calculateTimeToGoal(profile, calculation.deficitOrSurplus) : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* เป้าหมายเฉพาะ */}
      {profile.targetWeight && profile.timeframe && profile.goal !== 'maintain' && (
        <Card 
          style={{
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-medium text-ocean">
                    {formatWeight(Math.abs(profile.weight - profile.targetWeight))} กก.
                  </span>
                </div>
                <div className="flex justify-between items-center">
              ...
              {timeToGoal > 0 && timeToGoal !== profile.timeframe && (
                <div className="p-4 bg-warning/10 text-warning-foreground rounded-xl border border-warning/20">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span>
                      ตามแผนแคลอรี่ที่คำนวณ คาดว่าจะใช้เวลา {timeToGoal} สัปดาห์
                    </span>
                  </div>
                </div>
              )}
        ...
        <CardContent>
          ...
          {calculation.deficitOrSurplus !== 0 && (
            <div className="mt-6 p-4 rounded-xl border" style={{
              backgroundColor: calculation.deficitOrSurplus < 0 ? 'oklch(0.95 0.05 320 / 0.5)' : 'oklch(0.95 0.05 230 / 0.5)',
              borderColor: calculation.deficitOrSurplus < 0 ? 'oklch(0.65 0.2 320 / 0.3)' : 'oklch(0.6 0.2 230 / 0.3)'
            }}>
              <div className="text-center">
                <div className="flex items-center justify-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium">
                    {calculation.deficitOrSurplus < 0 ? 'Deficit' : 'Surplus'}: {formatCalories(Math.abs(calculation.deficitOrSurplus))} แคลอรี่/วัน
                  </span>
                </div>
                <div className="text-xs text-muted-foreground space-y-1">
                  <div>
                    คาดการณ์การเปลี่ยนแปลงน้ำหนัก: {formatWeight(Math.abs(weeklyWeightChange))} กก./สัปดาห์
                  </div>
                  {profile.targetWeight && profile.timeframe && (
                    <div>
                      เป้าหมาย: {formatWeight(Math.abs(profile.weight - profile.targetWeight))} กก. ใน {profile.timeframe} สัปดาห์
                    </div>
                  )}
                </div>
              </div>
        ...
        <CardContent className="space-y-6">
          <div>
            <div className="flex justify-between mb-3">
              ...
              <span className="font-medium text-lavender">{calculation.macroBreakdown.protein} กรัม (30%)</span>
            </div>
            <div className="h-3 bg-muted rounded-full overflow-hidden">
              <div className="h-full w-[30%] bg-lavender rounded-full"></div>
            </div>
          </div>
          <div>
            <div className="flex justify-between mb-3">
              <span className="font-medium text-sky">คาร์โบไหเดรต</span>
              <span className="font-medium text-sky">{calculation.macroBreakdown.carbs} กรัม (40%)</span>
            </div>
            <div className="h-3 bg-muted rounded-full overflow-hidden">
              <div className="h-full w-[40%] bg-sky rounded-full"></div>
            </div>
          </div>
          <div>
            <div className="flex justify-between mb-3">
              <span className="font-medium text-rose">ไขมัน</span>
              <span className="font-medium text-rose">{calculation.macroBreakdown.fat} กรัม (30%)</span>
            </div>
            <div className="h-3 bg-muted rounded-full overflow-hidden">
              <div className="h-full w-[30%] bg-rose rounded-full"></div>
            </div>
          </div>
        </CardContent>
      </Card>
