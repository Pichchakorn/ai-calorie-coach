...
import { formatCalories, formatWeight, getGoalText, getActivityText, calculateTargetCalories, calculateTDEE, generateMealPlan } from '../utils/calculations';
import { 
  Calculator, 
  Target, 
  TrendingUp, 
...
export function Dashboard({ onCreatePlan, onNavigateToSettings, recentPlans = [] }: DashboardProps) {
  ...
  const getGoalIcon = (goal: UserProfile['goal']) => {
    switch (goal) {
      ...
      case 'maintain': return <Minus className="h-4 w-4 text-deep-blue" />;
    }
  };

  const getGoalProgress = () => {
    if (!user?.profile?.targetWeight || !user?.profile?.timeframe) return null;
    
    const { weight, targetWeight, timeframe } = user.profile;
    const totalChange = Math.abs(targetWeight - weight);
    const weeksPassed = 2; // Simulated - in real app would calculate from start date
    const progressPercentage = Math.min((weeksPassed / timeframe) * 100, 100);
  ...
  const getHealthStats = () => {
    ...
    const tdee = calculateTDEE(0, user.profile.activityLevel);
    const calorieCalc = calculateTargetCalories(user.profile, tdee);
          ...
          {user?.profile && (
            ...
            >
              <CardHeader>
                ...
                <CardDescription>
                  ข้อมูลปัจจุบันและเป้าหมายของคุณ
                </CardDescription>
              </CardHeader>
              <CardContent>
          ...
          {goalProgress && (
            ...
            >
              <CardHeader>
                ...
                <CardDescription>
                  การติดตามเป้าหมายน้ำหนักของคุณ
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex justify-between items-center">
                  <span>ความคืบหน้า:</span>
                  <span className="text-sunset font-medium">{goalProgress.weeksPassed} / {goalProgress.timeframe} สัปดาห์</span>
                </div>
                <div className="relative">
                  ...
                  <div 
                    ...
                    style={{ 
                      background: 'linear-gradient(135deg, oklch(0.6 0.2 230), oklch(0.75 0.18 330), oklch(0.7 0.15 280))',
              ...
              {recentPlans.length > 0 ? (
                <div className="space-y-3">
                  {recentPlans.slice(0, 3).map((plan, index) => (
                    <div key={index} className="flex items-center justify-between p-4 bg-muted/30 rounded-xl border border-muted">
                      <div>
                        <div className="font-medium">แผนอาหาร {plan.mealPlan.date}</div>
                        <div className="text-sm text-deep-blue">
                          {plan.mealPlan.totalCalories} แคลอรี่
                        </div>
                      </div>
                ...
                <div className="pt-4 border-t">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between text-sm p-2 bg-sky/10 rounded border border-sky/20">
                      <span className="text-sky">คาร์โบไหเดรต</span>
